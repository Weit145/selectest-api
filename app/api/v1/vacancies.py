...
from app.utils.convert import db_to_read_vacancy,list_db_to_read_vacancy
...
async def list_vacancies_endpoint(
    ...
    return list_db_to_read_vacancy(await list_vacancies(session, timetable_mode_name, city))
...
async def get_vacancy_endpoint(
    ...
    return db_to_read_vacancy(vacancy)
...
async def create_vacancy_endpoint(
    ...
    return db_to_read_vacancy(await create_vacancy(session, payload))
...
async def update_vacancy_endpoint(
    ...
    if not vacancy:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found")
    return db_to_read_vacancy(await update_vacancy(session, vacancy, payload))
